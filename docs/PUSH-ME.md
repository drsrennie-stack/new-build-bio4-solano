...
**Point the Canvas iframe at `welcome.html`.** `welcome.html` is now a redirect to it, kept so any existing link survives. Safe to delete if nothing points at it.
