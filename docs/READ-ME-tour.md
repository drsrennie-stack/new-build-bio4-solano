...
The InteDashboard button is on `week-1.html`, but your home cards link to `week-1.html`, and the hub does not link onward to `week-1.html`. The tour jumps there on its own for the last step. If the hub is your real student page, the cleaner fix is to put the InteDashboard button on the hub. Say the word and I will move it.
