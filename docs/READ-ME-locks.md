...
The script keys off the file name, so `week-2.html` is locked automatically once that line is in it.
...
## Note
`index.html` still references `course-tour.js` and links to `week-1.html` … `week-8.html`.
